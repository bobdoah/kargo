...
When `mergeMethod` is not specified, the merge is performed using the default
merge method configured on the repository by the hosting provider. Specifying
an unsupported merge method for a provider will result in an error.
